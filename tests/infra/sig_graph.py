...
for sig_tx_interval in sig_tx_intervals:
    ...
    for i in range(5):
        ...
        throughputs[sig_tx_interval].append(stats["all_clients_active_average_throughput_tx/s"] / 1000)
...
min_errs = []
for sig_tx_interval in sig_tx_intervals:
    medium = np.percentile(throughputs[sig_tx_interval],50)
    heights.append(medium)
    max_errs.append(np.max(throughputs[sig_tx_interval]) - medium)
    min_errs.append(medium - np.min(throughputs[sig_tx_interval]))
